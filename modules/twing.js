...
exports.socketOnConnectionCallback = function (ioin, socket) {
  ...
  socket.on('register me', function (fbMe) {
    ...
    fbMe.name = fbMe.name
      ? fbMe.name.replace("'", '-')
      : jabber.createWord(5, true) + ' ' + jabber.createWord(6, true);
      ...
      ScoreM.findOne({ fbID: fbMe.id }).exec(function (err, myStats) {
        if (!err) {
          ...
          if (!myStats) {
            ...
            ScoreM.create(
              { fbID: fbMe.id, name: fbMe.name, score: 0, money: 200 },
              function (err, result) {
                if (!err) {
    ...
    socket.emit('userlist', {
      me: users[socket.id],
      roomName: 'lobby',
      users: usersInRoom(me.room, me.id),
    }); //2nd param to exclude me
  ...
  function createRoom(data) {
    ...
    rooms[room] = {
      name: data.roomID
        ? 'Dual-' +
          duels[data.roomID].challenger.name +
          ' Vs. ' +
          duels[data.roomID].target.name +
          ' $' +
          data.bet
        : room,
      host: me.name,
  ...
  function joinRoom(room) {
    if (rooms.hasOwnProperty(room)) {
      ...
      socket.emit(
        'problem',
        'Host not found. Other players may have left the host.'
      );
    }
  }
  ...
  socket.on('challenge', function (data) {
    ...
    io.sockets
      .socket(target.id)
      .emit('challenge', me, duels[socket.id + '-' + target.id]);
  });
  ...
  socket.on('matched', function (block) {
    ...
    if (block % 11 == 0 || block > 39) newScore += 150;
  ...
  socket.on('game over', function () {
    ...
    if (!rooms[me.room].gameOver) {
      ...
      if (me.currScore >= 3000 && roomMates.length == 5) {
        FB.api(
          me.fbMe.id + '/achievements',
          'post',
          {
            achievement:
              'http://review.com.np/og/achievement.php?achievement=' +
              rooms[me.room].theme.name +
              '',
          },
          function (res) {
            if (!res || res.error) {
      ...
      if (rooms[me.room].bet) {
        ...
        scoresArray.forEach(function (score, i) {
          ...
          console.log(calcMoney);
          users[scoresArray[i].id].betMoney =
            calcMoney < -rooms[me.room].bet ? rooms[me.room].bet : calcMoney;
  ...
  socket.on('get appFriends', function (appFriends) {
    ScoreM.find({ fbID: { $in: appFriends } })
      .sort('-score')
      .limit(150)
  ...
  socket.on('notifications', function () {
    if (ActionsM) {
      ActionsM.find({ targetID: users[socket.id].fbMe.id }).exec(function (
        err,
        notifications
      ) {
        if (err) {
          console.log(err);
        } else socket.emit('notifications', notifications);
      });
    }
  });
  socket.on('accept gift', function (id) {
    ActionsM.findById(id, {}, function (err, result) {
      if (!err) {
        if (result) {
          if (result.amount < 10) result.amount = 10;
          ScoreM.findOneAndUpdate(
            { fbID: result.targetID },
            { $inc: { money: result.amount } },
            { upsert: false },
            function (err, result) {
              if (!err) {
  ...
  socket.on('gift', function (data) {
    ...
    if (users.hasOwnProperty(data.target.sid)) {
      ...
      io.sockets
        .socket(target.id)
        .emit('gift', { sender: me, gift: 'coins10' });
    }
  });
...
function updateLeaversRoom(socket) {
  ...
  if (leaver.hasOwnProperty('room')) {
    ...
    if (rooms.hasOwnProperty(leaver.room)) {
      rooms[leaver.room].players.splice(
        rooms[leaver.room].players.indexOf(leaver.name),
        1
      );
...
function usersInRoom(room) {
  var usersInRoom = {};
  for (var userID in users) {
    if (room == users[userID].room) usersInRoom[userID] = users[userID];
  }
  if (arguments[1]) {
